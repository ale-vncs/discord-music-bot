...
export class FilterStrategy extends AbstractCommandStrategy<string> {
  ...
  async processMessage() {
    ...
    if (filters.includes('list')) {
      this.sendMessage(this.getListFilterName().join(' :: '))
      return
    }

    const filterValidate = this.validateFilter(filterList)
    ...
    const encoderList = this.getEncoderByFilterList(filterValidate)
    this.logger.info('encoded a ser aplicado: {}', encoderList)

    this.getSongManager().setFilter(encoderList)
  ...
  private getEncoderByFilterList(filters: string[]) {
    const list = ['-af']
    const enc = filters.map((f) => this.filterList()[f]).join(',')
    list.push(enc)
    return list
  }

  private validateFilter(filters: string[]) {
    return filters.filter((f) => this.getListFilterName().includes(f))
  }

  private getListFilterName() {
